...
//--------------------------------------------------------------------------------------
HRESULT CTerrain::OnCreateDevice( ID3D11Device* dev, ID3D11DeviceContext* con )
{
    HRESULT hr = S_OK;
    m_pDev10 = con;
	m_pDev10dev = dev;
    ...
    V_RETURN( dev->CreateBuffer( &BufferDesc, &InitData, &m_pTerrainIB10 ) );
...
void CTerrain::RenderTile( TERRAIN_TILE* pTile )
{
        ...
        UINT offset = 0;
        m_pDev10->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP );
...
void CTerrain::RenderGrass( D3DXVECTOR3* pViewDir, UINT NumInstances )
{
    ...
    for( UINT i = 0; i < m_NumDirections; i++ )
    {
        ...
        m_pDev10->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
...
HRESULT CTerrain::GenerateTile( TERRAIN_TILE* pTile, BOUNDING_BOX* pBBox )
{
    ...
    for( UINT z = 0; z < m_NumSidesPerTile + 1; z++ )
    {
        ...
        for( UINT x = 0; x < m_NumSidesPerTile + 1; x++ )
        {
            ...
            //pTile->pRawVertices[iVertex].color = D3DXVECTOR4(1.0f, 1.0f, 1.0f, 1.0f); // pridane
...
HRESULT CTerrain::CreateTileResources( TERRAIN_TILE* pTile )
{
    HRESULT hr = S_OK;
        ...
        V_RETURN( m_pDev10dev->CreateBuffer( &BufferDesc, &InitData, &pTile->pVB10 ) );
...
HRESULT CTerrain::CreateGrass()
{
    ...
    for( UINT i = 0; i < m_NumGrassBlades; i++ )
    {
        ...
        V_RETURN( m_pDev10dev->CreateBuffer( &BufferDesc, &InitData, &m_pGrassVB10 ) );
    ...
    m_NumDirections = 16;
        ...
        m_ppGrassIB10 = new ID3D11Buffer*[ m_NumDirections ];
        if( !m_ppGrassIB10 )
            return E_OUTOFMEMORY;
    ...
    for( UINT i = 0; i < m_NumDirections; i++ )
    {
        ...
        for( UINT g = 0; g < m_NumGrassBlades; g++ )
        {
            ...
            V_RETURN( m_pDev10dev->CreateBuffer( &BufferDesc, &InitData, &m_ppGrassIB10[i] ) );
