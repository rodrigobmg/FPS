...
private :
	...
	void Gravitation();	
	void TestCollisions();
	void BspCollision(Entity::Base *a);
	void ColAnswer(Entity::Base *a, Entity::Base *b);
	void FinalForce();	
};

// Entities* movedentities; 

// TODO *Ak entita ma stale velocity, cize dalsi snimok bude aj tak pohyb, uloz do m_dynamic/m_vynimka a tie prehladavaj ku kazdemu a yo sluzi aj pre obrovske objekty
// - A napriklad len m_dynamic kazdej entite vyvtori AABBMM vlastne
...
void PhysicsManager::ColAnswer(Entity::Base *a, Entity::Base *b) {
	// Vypocitaj odpoved
	// Zavolaj tocuh na A aj B
	// Aplikuj na obidve entity
	// Nastav nove F
}
void PhysicsManager::BspCollision(Entity::Base *a) {
