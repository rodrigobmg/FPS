						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../Media/fontdata.txt", L"../Media/font.dds");
	if(!result)
	{
	...
	result = UpdateSentence(m_sentence1, "Hello", 300, 300, 1.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 500, 500, 0.0f, 1.f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool Text::SetRenderCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence1, countString, 20, 20, 0.0f, 0.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
