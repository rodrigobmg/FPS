...
Refraction::Refraction(const Refraction& other)
{
}
...
bool Refraction::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Media/refraction.vs", L"../Media/refraction.ps");
	if(!result)
	{
		return false;
	}
...
bool Refraction::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, 
...
bool Refraction::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void Refraction::ShutdownShader()
{
...
void Refraction::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool Refraction::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
void Refraction::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
