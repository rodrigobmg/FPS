...
// Graphics Gems.
//-----------------------------------------------------------------------------
VOID ComputeBoundingSphereFromPoints( Sphere* pOut, UINT Count, const vec* pPoints, UINT Stride )
{
    XMASSERT( pOut );
    XMASSERT( Count > 0 );
    XMASSERT( pPoints );
    ...
    {
        XMVECTOR Point = XMLoadFloat3( ( vec* )( ( BYTE* )pPoints + i * Stride ) );
...
//-----------------------------------------------------------------------------
VOID ComputeBoundingAxisAlignedBoxFromPoints( AxisAlignedBox* pOut, UINT Count, const vec* pPoints, UINT Stride )
{
    XMASSERT( pOut );
    XMASSERT( Count > 0 );
    XMASSERT( pPoints );
...
//-----------------------------------------------------------------------------
VOID ComputeBoundingOrientedBoxFromPoints( OrientedBox* pOut, UINT Count, const vec* pPoints, UINT Stride )
{
    ...
    for( UINT i = 0; i < Count; i++ )
    {
        XMVECTOR Point = XMLoadFloat3( ( vec* )( ( BYTE* )pPoints + i * Stride ) ) - CenterOfMass;
    ...
    for( UINT i = 1; i < Count; i++ )
    {
        XMVECTOR Point = XMVector3TransformNormal( XMLoadFloat3( ( vec* )( ( BYTE* )pPoints + i * Stride ) ),
                                                   InverseR );
...
VOID ComputeFrustumFromProjection( Frustum* pOut, XMMATRIX* pProjection )
{
    ...
    pOut->Origin.x =  pOut->Origin.y =  pOut->Origin.z = 0.0f;
    pOut->Orientation.x = pOut->Orientation.y = pOut->Orientation.z = pOut->Orientation.w = 0.0f;
